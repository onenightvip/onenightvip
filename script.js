...
async function fetchGeographyData() {
    try {
        const res = await fetch('https://raw.githubusercontent.com/kongvoon/thai-province-data/master/api_province_with_amphure.json');
        fullGeographyData = await res.json();
    } catch(e) { console.error("โหลดข้อมูลอำเภอไม่สำเร็จ", e); }
}
...
function updateDistrictList(provinceName) {
    const distList = document.getElementById('distList');
    const distInput = document.getElementById('mDist');
    if(!distList || !distInput) return;
    
    distList.innerHTML = ''; 
    distInput.value = ''; // ล้างช่องอำเภอเมื่อเปลี่ยนจังหวัด

    const provData = fullGeographyData.find(p => p.name_th === provinceName);
    if(provData && provData.amphure) {
        provData.amphure.forEach(a => {
            let option = document.createElement('option');
            option.value = a.name_th;
...
document.addEventListener('DOMContentLoaded', () => {
    fetchGeographyData(); // โหลดข้อมูลอำเภอซ่อนไว้เบื้องหลัง
    setupProvinceAutocomplete();
    
    const provInput = document.getElementById('mProv');
    if(provInput) {
        // ทำงานเมื่อผู้ใช้พิมพ์เสร็จ หรือเลือกจังหวัด
        provInput.addEventListener('change', function(e) {
            let val = e.target.value.trim();
            // ระบบช่วยแก้คำผิด
            if(val === 'กทม' || val === 'กทม.') val = 'กรุงเทพมหานคร';
            else if(val === 'กรุงเทพ') val = 'กรุงเทพมหานคร';
            else if(val === 'โคราช') val = 'นครราชสีมา';
            else if(val === 'เมืองชล') val = 'ชลบุรี';
            e.target.value = val;
            
            updateDistrictList(val); // ดึงข้อมูลอำเภอมาใส่
        });
    }
});
...
// 🔥 แก้ไขบัก QR 401: เพิ่ม Authorization แนบไปกับคำสั่ง 🔥
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/create-qr`, { 
            ...
            headers: { 
                ...
                // ใส่กุญแจ Anon Key เพื่อให้ผ่านด่าน รปภ. ของ Supabase
        ...
        if (!response.ok) {
            ...
            throw new Error(`การเชื่อมต่อมีปัญหา (Status ${response.status}) - ${errText}`);
        }
