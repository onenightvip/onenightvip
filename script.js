...
function setupProvinceAutocomplete() { 
    const dataList = document.getElementById('provListOptions'); 
    if(dataList) { 
        dataList.innerHTML = '';
        thaiProvinces.forEach(prov => { 
            dataList.innerHTML += `<div class="custom-option-item" onclick="selectCustomOption('prov', '${prov}')">${prov}</div>`;
        }); 
    } 
}
function updateDistrictList(provinceName) { 
    const distList = document.getElementById('distListOptions'); 
    if(!distList) return; 
    distList.innerHTML = ''; 
    const provData = fullGeographyData.find(p => p.name_th === provinceName); 
    if(provData && provData.amphure) { 
        provData.amphure.forEach(a => { 
            distList.innerHTML += `<div class="custom-option-item" onclick="selectCustomOption('dist', '${a.name_th}')">${a.name_th}</div>`;
        }); 
    } 
}
// 🔥 ควบคุมระบบ Custom Dropdown 🔥
function toggleCustomDropdown(id) {
    document.querySelectorAll('.custom-options-container').forEach(el => { if(el.id !== id) el.classList.remove('show'); });
    document.getElementById(id).classList.toggle('show');
}
function filterCustomOptions(listId, searchText) {
    const text = searchText.toLowerCase();
    document.querySelectorAll(`#${listId} .custom-option-item`).forEach(el => {
        el.style.display = el.innerText.toLowerCase().includes(text) ? 'block' : 'none';
    });
}
function selectCustomOption(type, value) {
    if(type === 'prov') {
        document.getElementById('provDisplay').innerText = value; document.getElementById('provDisplay').style.color = '#fff';
        document.getElementById('mProv').value = value;
        updateDistrictList(value);
        document.getElementById('distDisplay').innerText = 'เลือกเขต/อำเภอ...'; document.getElementById('distDisplay').style.color = '#888';
        document.getElementById('mDist').value = '';
        document.getElementById('provDropdown').classList.remove('show');
    } else if(type === 'dist') {
        document.getElementById('distDisplay').innerText = value; document.getElementById('distDisplay').style.color = '#fff';
        document.getElementById('mDist').value = value;
        document.getElementById('distDropdown').classList.remove('show');
    }
}
// ดักจับการคลิกที่อื่นเพื่อปิดกล่อง
document.addEventListener('click', (e) => {
    if(!e.target.closest('.custom-select-wrapper') && !e.target.closest('.custom-options-container')) {
        document.querySelectorAll('.custom-options-container').forEach(el => el.classList.remove('show'));
    }
});
...
async function fetchModels() { 
    // 🔒 เพิ่มกุญแจดอกที่ 1: ดึงมาโชว์เฉพาะคนที่แอดมินอนุมัติแล้ว (is_verified = true)
