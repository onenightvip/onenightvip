...
let currentUserSession = null; let allModelsData = []; let myAgencyModels = []; let userFavorites = []; let currentProfileId = null; let selectedRating = 5; let agencyWalletBalance = 0; let isEditing = false; let dashboardChartInstance = null;
let myCurrentTheme = 'default'; // 🔥 เก็บค่าธีม
...
function updateUIAuth(session) {
    ...
    if (isLoggedIn && session.user) {
        fetchUserFavorites(); fetchWalletBalanceAndTheme(); 
...
// 🔥 ดึงยอดเงินพร้อมกับธีมสี 🔥
async function fetchWalletBalanceAndTheme() {
    if(!currentUserSession) return;
    const { data, error } = await supabaseClient.from('user_profiles').select('wallet_balance, theme_color').eq('id', currentUserSession.user.id).single();
    if(!error && data) { 
        ...
        applyTheme(myCurrentTheme); // สั่งเปลี่ยนสีแดชบอร์ดให้เป็นสีของตัวเอง
    }
}

// 🔥 ฟังก์ชันสลับสีธีมหน้าเว็บ 🔥
function applyTheme(themeName) {
    document.body.className = document.body.className.replace(/theme-\w+/g, '').trim(); // ล้างธีมเก่า
    if(themeName && themeName !== 'default') {
        document.body.classList.add(`theme-${themeName}`); // ใส่ธีมใหม่
    }
}

// 🔥 ฟังก์ชันเซฟธีมเมื่อกดปุ่มเปลี่ยนสี 🔥
async function saveTheme(themeName, btnElement) {
    if(!currentUserSession) return;
    const oldBorder = btnElement.style.borderColor;
    btnElement.style.borderColor = '#10b981'; // เปลี่ยนสีขอบเป็นเขียวชั่วคราว
    
    const { error } = await supabaseClient.from('user_profiles').update({ theme_color: themeName }).eq('id', currentUserSession.user.id);
    if(!error) {
        myCurrentTheme = themeName;
        applyTheme(themeName);
        setTimeout(() => { btnElement.style.borderColor = oldBorder; }, 1000);
...
function openDashboardRouter() {
    ...
    else if (role === 'agency' || role === 'freelance' || role === 'Owner') { document.getElementById('agencyDashboard').classList.add('active'); applyTheme(myCurrentTheme); fetchMyProfiles(); fetchWalletBalanceAndTheme(); setTimeout(() => { renderDashboardChart(); }, 100); } 
...
function setupAdminDashboard(role) { document.getElementById('adminTierName').innerText = role; const navMenu = document.getElementById('adminNavMenu'); navMenu.innerHTML = `<div class="dash-nav-item active" onclick="switchAgTab('admin-overview', this, 'adminDashboard')"><span class="iconify" data-icon="heroicons:squares-2x2"></span> ภาพรวมระบบ</div>`; if(role === 'Super Admin') { navMenu.innerHTML += `<div class="dash-nav-item" onclick="switchAgTab('admin-approve', this, 'adminDashboard')"><span class="iconify" data-icon="heroicons:check-badge"></span> อนุมัติโปรไฟล์</div>`; navMenu.innerHTML += `<div class="dash-nav-item" onclick="switchAgTab('admin-wallet', this, 'adminDashboard')"><span class="iconify" data-icon="heroicons:currency-dollar"></span> จัดการเครดิต (Wallet)</div>`; navMenu.innerHTML += `<div class="dash-nav-item" onclick="switchAgTab('admin-system', this, 'adminDashboard')"><span class="iconify" data-icon="heroicons:code-bracket"></span> ตั้งค่าระบบ</div>`; navMenu.innerHTML += `<div class="dash-nav-item" onclick="switchAgTab('admin-audit', this, 'adminDashboard')"><span class="iconify" data-icon="heroicons:clipboard-document-list"></span> Audit Logs</div>`; } else if (role === 'Developer') { navMenu.innerHTML += `<div class="dash-nav-item" onclick="switchAgTab('admin-system', this, 'adminDashboard')"><span class="iconify" data-icon="heroicons:code-bracket"></span> ตั้งค่าระบบ</div>`; } else if (role === 'Admin') { navMenu.innerHTML += `<div class="dash-nav-item" onclick="switchAgTab('admin-approve', this, 'adminDashboard')"><span class="iconify" data-icon="heroicons:check-badge"></span> อนุมัติโปรไฟล์</div>`; } supabaseClient.from('user_profiles').select('id', { count: 'exact' }).then(({count}) => document.getElementById('adminTotalUsers').innerText = count || 0); supabaseClient.from('models').select('id', { count: 'exact' }).then(({count}) => document.getElementById('adminTotalModels').innerText = count || 0); fetchAdminData(role); }
...
// 🔥 อัปเกรดระบบดึงข้อมูลแอดมิน กันบั๊กฐานข้อมูล 🔥
async function fetchAdminData(role) {
    if(role === 'Super Admin' || role === 'Admin') {
        // ดึงข้อมูลน้องๆ ที่รออนุมัติ (is_verified = false)
        ...
        (pendingModels||[]).forEach(m => {
            ...
            if(m.kyc_status === 'pending') { 
                btnHtml = `<button class="btn-glow btn-glow-sm" style="margin-bottom:5px;" onclick="window.open('${m.kyc_image}', '_blank')">🔍 ดูรูป KYC</button>` + btnHtml; 
            }
...
async function submitNewModel(event) { event.preventDefault(); if(!currentUserSession) return; if(mediaFiles.length === 0 && existingGallery.length === 0) { alert("ต้องมีรูปภาพอย่างน้อย 1 รูปครับ!"); return; } const btn = document.getElementById('btnSubmitModel'); const originalHtml = btn.innerHTML; btn.innerHTML = '<span class="iconify" data-icon="eos-icons:bubble-loading"></span> กำลังประมวลผล...'; let uploadedUrls = []; for (let item of mediaFiles) { const fExt = item.file.name.split('.').pop(); const fName = currentUserSession.user.id + '/' + Math.random().toString(36).substring(2) + '.' + fExt; const { error } = await supabaseClient.storage.from('profile_images').upload(fName, item.file); if (!error) uploadedUrls.push(supabaseClient.storage.from('profile_images').getPublicUrl(fName).data.publicUrl); } const finalGallery = [...existingGallery, ...uploadedUrls]; const coverImage = finalGallery[0]; const chest = document.getElementById('mChest').value; const waist = document.getElementById('mWaist').value; const hips = document.getElementById('mHips').value; const props = `${chest}-${waist}-${hips}`; const locationStr = document.getElementById('mProv').value + " " + document.getElementById('mDist').value + " " + (document.getElementById('mVenue').value || ""); const modelData = { agency_id: currentUserSession.user.id, name: document.getElementById('mName').value, age: document.getElementById('mAge').value, slogan: document.getElementById('mSlogan').value, price: document.getElementById('mPrice').value, languages: document.getElementById('mLang').value, province: document.getElementById('mProv').value, district: document.getElementById('mDist').value, location: locationStr.trim(), gender: document.getElementById('mGen').value, height: document.getElementById('mHeight').value, weight: document.getElementById('mWeight').value, chest: chest, cup_size: document.getElementById('mCup').value, breast_type: document.getElementById('mBreastType').value, waist: waist, hips: hips, proportions: props, line_id: document.getElementById('mLineId').value, telegram_id: document.getElementById('mTele').value, twitter_id: document.getElementById('mTwit').value, description: document.getElementById('mDesc').value, cover_image: coverImage, gallery: finalGallery, status: 'active' }; const editingId = document.getElementById('editingModelId').value; let errorMsg = null; if(isEditing && editingId) { const { error } = await supabaseClient.from('models').update(modelData).eq('id', editingId); errorMsg = error ? error.message : null; } else { modelData.is_verified = false; modelData.kyc_status = 'none'; const { error } = await supabaseClient.from('models').insert([modelData]); errorMsg = error ? error.message : null; } if (errorMsg) { alert('❌ Error: ' + errorMsg); } else { alert(isEditing ? '✅ อัปเดตข้อมูลสำเร็จ!' : '🎉 สร้างโปรไฟล์สำเร็จ! (รอแอดมินตรวจสอบ)'); resetForm(); fetchMyProfiles(); fetchModels(); } btn.innerHTML = originalHtml; }
...
async function generateRealQR() {
    ...
    const btn = document.getElementById('btnGenQR'); const originalHtml = btn.innerHTML; btn.innerHTML = '<span class="iconify" data-icon="eos-icons:bubble-loading"></span> กำลังสร้าง QR Code...';
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/create-qr`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` }, body: JSON.stringify({ amount: amount, agency_id: currentUserSession.user.id }) });
        if (!response.ok) { const errText = await response.text(); throw new Error(`การเชื่อมต่อมีปัญหา (Status ${response.status}) - ${errText}`); }
        const data = await response.json(); if (data.error) { throw new Error(data.error); }
        
        document.getElementById('omiseQrImage').src = data.qr_url; document.getElementById('qrAmountDisplay').innerText = amount.toLocaleString(); document.getElementById('qrModalOverlay').classList.add('active');
        alert("🎉 สร้าง QR Code จาก Omise สำเร็จ!\n(สามารถจำลองการจ่ายได้ในเว็บ Omise เลยครับ)");

        const oldBalance = agencyWalletBalance;
        const checkInterval = setInterval(async () => {
            if(!document.getElementById('qrModalOverlay').classList.contains('active')) { clearInterval(checkInterval); return; }
            await fetchWalletBalanceAndTheme();
            if(agencyWalletBalance > oldBalance) { clearInterval(checkInterval); alert(`✅ การชำระเงินสำเร็จ!\nระบบได้เติมเครดิต ${amount} บาท เข้ากระเป๋าเรียบร้อยแล้วครับ`); closeQRModal(); document.getElementById('customTopupAmount').value = ''; }
        }, 3000);
    } catch (error) { console.error(error); alert("❌ เกิดข้อผิดพลาดจากธนาคาร: " + error.message); }
    btn.innerHTML = originalHtml;
}
...
// 🔥 โหลดโมเดลพร้อมกับดึงข้อมูล theme ของเอเจนซี่มาด้วย 🔥
async function fetchModels() { 
    // ใช้คำสั่ง join ตาราง user_profiles เพื่อเอา theme_color มา
